...
{
  EPD_WR_REG(0x22);  /* Display Mode Control command. For details, refer to the SSD1680 datasheet. */
  
  /* If the clock and DC-DC converter were already enabled before the partial refresh 
     and have not been turned off, we do not need to enable them again here. 
     Otherwise, they MUST be turned on!!! */
     
  /* Action sequence: Turn on clock -> Turn on DC-DC -> Read ambient temperature -> 
     Load Look-Up Table (LUT) -> Work in partial refresh mode -> Execute image refresh -> 
     Keep DC-DC and clock enabled. */
  EPD_WR_DATA8(0xFC); /* To prevent errors for users unfamiliar with the workflow, this is configured 
                         to always turn everything on by default. If it was already turned on previously, 
                         this value can be changed to 0x1C to save speed/power. */
  EPD_WR_REG(0x20);
  EPD_READBUSY();
